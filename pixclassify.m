function out=pixclassify(h,frame)
...
if numel(obj.tree)~=0
   ...
   % figure, imshow(subgfp,[]);
    ...
    limi=stretchlim(tmpim,[0.1 0.995]);
...
%out.classi=zeros(size(h.UserData.classi));
%size(imoutrgb)
out.classi(:,:,:,frames)=imoutrgb;

out.traintrack=uint8(zeros(size(out.classi))); % clear object training

%obj.traintrack=permute(obj.traintrack,[1 2 4 3]);

out.traintrack(:,:,2,frames)=0.5*out.classi(:,:,2,frames); % displays training data for tracking

%figure, imshow(out.classi(:,:,:,frames),[])
%drawnow
%figure(h)
%toc;
